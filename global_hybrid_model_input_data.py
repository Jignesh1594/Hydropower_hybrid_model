...
def process_global_hydropower_data(output_filepath=None, start_year = 2000, end_year = 2022):
    """
    Processes and integrate global hydropower data
    ...
    logger.info("Loading Europe (ENTSOE) generation data to get installed capacity for Europe plants")

    europe_data = pd.read_excel(f"{base_path}GloHydroRes_updates/ENTSOE_installed_capacity_glohydrores_plant_id.xlsx")
    europe_data = europe_data[["glohydrores_plant_id", "installed_capacity_mw"]]
    
    # Create the mapping dictionary
    capacity_dict = dict(zip(europe_data["glohydrores_plant_id"], europe_data["installed_capacity_mw"]))

    # Apply the mapping and keep existing values where there's no match
    GloHydroRes["capacity_mw"] = GloHydroRes["ID"].map(capacity_dict).fillna(GloHydroRes["capacity_mw"])

    logger.info("Loading observed maximum generation data to update the capacity of plants")

    # Load observed maximum generation data
    observed_max_gen = pd.read_excel(f"{base_path}GloHydroRes_updates/updated_installed_capacity_based_on_observed_generation.xlsx")
    observed_max_gen = observed_max_gen[["glohydrores_plant_id", "installed_capacity_mw"]]

    # Create the mapping dictionary
    capacity_dict = dict(zip(observed_max_gen["glohydrores_plant_id"], observed_max_gen["installed_capacity_mw"]))

    # Apply the mapping and keep existing values where there's no match
    GloHydroRes["capacity_mw"] = GloHydroRes["ID"].map(capacity_dict).fillna(GloHydroRes["capacity_mw"])

    logger.info("Filtering out plants")

    # Remove pumped storage or canal plants
    GloHydroRes = GloHydroRes[~GloHydroRes["plant_type"].isin(["PS", "Canal"])]

    # Set reservoir volume to 0 for run-of-river plants with missing values
    GloHydroRes.loc[(pd.isna(GloHydroRes.res_vol_km3)) & 
                               (GloHydroRes.plant_type == "ROR"), "res_vol_km3"] = 0
    ...
    GloHydroRes["pcr_dam_lat_index"] = GloHydroRes["ID"].map(plant_ID_pcr_lat_dict)
    GloHydroRes["pcr_dam_lon_index"] = GloHydroRes["ID"].map(plant_ID_pcr_lon_dict)
    GloHydroRes = GloHydroRes[~GloHydroRes.pcr_dam_lat_index.isna()]

    logger.info("Remove columns that are not needed")
    cols = ["ID", "country",
            "name", "capacity_mw",
            "plant_lat", "plant_lon", "final_head_m", 
             "res_dam_source", "res_dam_source_id",
             "res_vol_km3", "pcr_dam_lat_index",
                "pcr_dam_lon_index"]
    
    GloHydroRes = GloHydroRes[cols]
    GloHydroRes.rename(columns={"ID" : "glohydrores_plant_id", "capacity_mw" : "installed_capacity_mw"}, inplace=True)

    logger.info("Convert to time series data")
    ...
    for _, plant in GloHydroRes.iterrows():
        plant_dict = plant.to_dict()

        for date in date_range:
            time_point = plant_dict.copy()
            time_point["date"] = date
            time_series_data.append(time_point)

    GloHydroRes_time_series = pd.DataFrame(time_series_data)

    # reset index
    GloHydroRes_time_series.reset_index(drop=True, inplace=True)
    GloHydroRes_time_series["Year"] = GloHydroRes_time_series["date"].dt.year
    GloHydroRes_time_series["Month"] = GloHydroRes_time_series["date"].dt.month

    if output_filepath:
        GloHydroRes_time_series.to_csv(output_filepath, index=False)
    
    return GloHydroRes_time_series
...
def physical_based_hydropower_func(discharge_path, df, start_year, lag_month):
    ...
    if np.any(pd.DatetimeIndex(times).day != 1):
        ...
        streamflow_data = streamflow_data.assign_coords(time = new_times)
    ...
    for idx in tqdm(range(len(df)), total=len(df), desc="Simulating phyiscal hydropower generation"):
...
def main():
    global_hybrid_model_training_data_all_plants = process_global_hydropower_data(start_year=1982, end_year=2022)
    
    lag_month = 6
    discharge_path = "/home/shah0012/PCRGLOWB_data/discharge/gswp3_w5e5/runs_till_2022/"
    
    simulated_hydro = physical_based_hydropower_func(discharge_path, global_hybrid_model_training_data_all_plants, "1981", lag_month)


    global_hybrid_model_training_data_all_plants = global_hybrid_model_training_data_all_plants.merge(simulated_hydro, on=["glohydrores_plant_id", "date"], how="left")
    min_year = str(global_hybrid_model_training_data_all_plants["Year"].min())
    max_year = str(global_hybrid_model_training_data_all_plants["Year"].max())
    ...
    global_hybrid_model_training_data_all_plants["country_code"] = global_hybrid_model_training_data_all_plants["country"].map(dict(zip(IRENA_wind_monthly["glohydrores_country"], IRENA_wind_monthly["shape_country_code"])))
    global_hybrid_model_training_data_all_plants["country_area"] = global_hybrid_model_training_data_all_plants["country"].map(dict(zip(IRENA_wind_monthly["glohydrores_country"], IRENA_wind_monthly["country_area"])))

    logger.info("Merging wind and solar installed capacity data to global generation data")

    global_hybrid_model_training_data_all_plants = global_hybrid_model_training_data_all_plants.merge(IRENA_wind_monthly[["glohydrores_country", "Date", "TotalWind"]], left_on=["country", "date"], right_on=["glohydrores_country", "Date"], how="left")
    global_hybrid_model_training_data_all_plants = global_hybrid_model_training_data_all_plants.merge(IRENA_solar_monthly[["glohydrores_country", "Date", "SolarPV"]], left_on=["country", "date"], right_on=["glohydrores_country", "Date"], how="left")
    global_hybrid_model_training_data_all_plants.rename(columns={"TotalWind": "IRENA_wind_installed_capacity", "SolarPV": "IRENA_solar_installed_capacity"}, inplace=True)
    ...
    #solar_cf_data = solar_cf_data.load()
    ...
    #wind_cf_data = wind_cf_data.load()
    ...
    wind_cf_country_means =  get_country_mean_variable(global_hybrid_model_training_data_all_plants, country_code, wind_cf_data, "wind_capacity_factor", country_shape)
    global_hybrid_model_training_data_all_plants = global_hybrid_model_training_data_all_plants.merge(wind_cf_country_means[["glohydrores_plant_id", "date", "wind_capacity_factor"]], on = ["glohydrores_plant_id", "date"], how = "left")

    solar_cf_country_means = get_country_mean_variable(global_hybrid_model_training_data_all_plants, country_code, solar_cf_data, "solar_capacity_factor", country_shape)
    global_hybrid_model_training_data_all_plants = global_hybrid_model_training_data_all_plants.merge(solar_cf_country_means[["glohydrores_plant_id", "date", "solar_capacity_factor"]], on = ["glohydrores_plant_id", "date"], how = "left")

    global_hybrid_model_training_data_all_plants["Solar_monthly_generation"] = global_hybrid_model_training_data_all_plants["solar_capacity_factor"] * global_hybrid_model_training_data_all_plants["IRENA_solar_installed_capacity"]*global_hybrid_model_training_data_all_plants["date"].dt.days_in_month * 24
    global_hybrid_model_training_data_all_plants["Wind_monthly_generation"] = global_hybrid_model_training_data_all_plants["wind_capacity_factor"] * global_hybrid_model_training_data_all_plants["IRENA_wind_installed_capacity"]*global_hybrid_model_training_data_all_plants["date"].dt.days_in_month * 24

    global_hybrid_model_training_data_all_plants["Solar_monthly_generation_persqkm"] = global_hybrid_model_training_data_all_plants["Solar_monthly_generation"]/global_hybrid_model_training_data_all_plants["country_area"]
    global_hybrid_model_training_data_all_plants["Wind_monthly_generation_persqkm"] = global_hybrid_model_training_data_all_plants["Wind_monthly_generation"]/global_hybrid_model_training_data_all_plants["country_area"]
    ...
    cdd_cf_country_means =  get_country_mean_variable(global_hybrid_model_training_data_all_plants, country_code, CDD_data, "CDD", country_shape)
    global_hybrid_model_training_data_all_plants = global_hybrid_model_training_data_all_plants.merge(cdd_cf_country_means[["glohydrores_plant_id", "date", "CDD"]], on = ["glohydrores_plant_id", "date"], how = "left")

    hdd_cf_country_means = get_country_mean_variable(global_hybrid_model_training_data_all_plants, country_code, HDD_data, "HDD", country_shape)
    global_hybrid_model_training_data_all_plants = global_hybrid_model_training_data_all_plants.merge(hdd_cf_country_means[["glohydrores_plant_id", "date", "HDD"]], on = ["glohydrores_plant_id", "date"], how = "left")
    ...
    temperature_country_means = calc_var_stat(global_hybrid_model_training_data_all_plants, temperature_data, "temperature", "latitude", "longitude")

    global_hybrid_model_training_data_all_plants = global_hybrid_model_training_data_all_plants.merge(temperature_country_means[["glohydrores_plant_id", "date", "temperature"]], on = ["glohydrores_plant_id", "date"], how = "left")

    col_names = ['country',
             'glohydrores_plant_id', 'date',
             "installed_capacity_mw", 
             'res_vol_km3', 'final_head_m',
    ...
    global_hybrid_model_training_data_all_plants[col_names].to_csv(f"{base_path}hybrid_model_data/global_model_train_data_1981_2022_selected_variables_all_plants.csv", index=False)
    global_hybrid_model_training_data_all_plants.to_csv(f"{base_path}hybrid_model_data/global_model_train_data_1981_2022_all_variables_all_plants.csv", index=False)


    # Update 
    return global_hybrid_model_training_data_all_plants


if __name__ == "__main__":
    global_hybrid_model_training_data_all_plants = main()
