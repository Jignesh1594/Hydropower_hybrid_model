...
"""
Global hydropower plants predictions with best performing machine learning model 
...
def main(train_file, valid_file, test_file, model_output_file, global_input_file, output_file,  min_observations = 24):
    logger.info(
    ...
    logger.info("Loading the best parameters for the Random Forest model")
    best_params = pickle.load(open(model_output_file + "optuan_tunned_RF_designed_split_target_cf.pkl", "rb"))
    ...
    logger.info("Global predictions")

    logger.info("Read Global input data file")
    global_data = pd.read_csv(global_input_file, parse_dates = ["date"])
    global_original_features = [x for x in features if x != "observed_generation_mw"]
    global_data = global_data.loc[:, global_original_features]


    # remove this line for 2000 to 2022
    global_data["Solar_monthly_generation_persqkm"] = global_data.Solar_monthly_generation_persqkm.fillna(0)
    global_data["Wind_monthly_generation_persqkm"] = global_data.Wind_monthly_generation_persqkm.fillna(0) 
    global_data = global_data.dropna(axis = 0)    
    global_data = global_data.apply(convert_to_float1632)
 
    global_unique_plants = global_data.glohydrores_plant_id.unique()

    logger.info("Standardized global input data features")

    for plant in global_unique_plants:
        plant_mask = global_data["glohydrores_plant_id"] == plant
        plant_data = global_data.loc[plant_mask, feature_cols]

        if plant in scalers:
             logger.info(f"Plant {plant} observed data available therefore taking scaler from that")
             scaler = scalers[plant]
             plant_scaled =  scaler.transform(plant_data.values)
             global_data.loc[plant_mask, output_cols] = plant_scaled

        else:
            logger.info(f"Plant {plant} observed data is not available")
            scaler = StandardScaler()
            plant_scaled = scaler.fit_transform(plant_data.values)
            global_data.loc[plant_mask, output_cols] = plant_scaled
            scalers[plant] = scaler
    ...
    global_features = [x for x in standardized_features if x != "observed_plant_CF"]
    global_data = global_data.loc[:, global_features]
    global_data  = global_data[global_data.installed_capacity_mw > 10]

    global_data['predicted_CF'] = model_final.predict(global_data.drop(columns=["date", "glohydrores_plant_id", "country"]))
    
    
    logger.info("Coverting predicted capacity factor to power (MW)")
    global_data["originial_prediction"] = global_data['predicted_CF'] * global_data['installed_capacity_mw']
